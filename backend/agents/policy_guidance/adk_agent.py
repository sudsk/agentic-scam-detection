# backend/agents/policy_guidance/adk_agent.py - FIXED VERSION
"""
Fixed Policy Guidance Agent with proper ADK integration
...
def create_policy_guidance_agent() -> Agent:
    """Create the policy guidance agent using Google ADK - FIXED VERSION"""
    
    # Get config without forbidden parameters  
    config = AGENT_CONFIGS.get("policy_guidance_agent", {})
    
    # FIXED: Use only allowed parameters
    return Agent(
        name="policy_guidance_agent",  # Simple string identifier
        ...
        tools=[],  # No tools for now - pure policy retrieval
    )
